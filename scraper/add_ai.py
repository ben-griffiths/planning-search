...
def get_related_industries(text):
    ...
    return json.loads(
        response_json["choices"][0]["message"]["content"].replace("'", '"')
    )
...
def process_document(document):
    ...
    document_content = document["_source"]

    # Prepare query for ChatGPT
    command = (
        "Which industry sic codes do you expect this planning application data to "
        "relate to and how relevant do you think it is as a score? "
        "(include as many scores that you beleive are relevant) "
        "(industry code and relation score list written in json and no dialog) "
        "use this as a template: "
        "[ { 'code': '56102', 'relation_score': 0.9 } ]"
        "Make sure the sic codes are in the following mapping: {SIC_MAPPING}"
    )
    query = f"{command}\n\n{document_content}"

    # Call ChatGPT API to get industry codes
    related_industries = get_related_industries(query)

    # Update document in OpenSearch with new field
    doc_body = {"doc": {"related_industries": related_industries}}
    opensearch.update(index="index", id=document_id, body=doc_body)

    print("Processed document:", document_id)
...
if __name__ == "__main__":
    ...
    response = opensearch.search(
        index="index",
        body={
            "query": {"exists": {"field": "related_industries"}},
            "size": 10000,
            "from": 0,
        },
    )
    ...
    # Process documents in parallel using the pool map function
